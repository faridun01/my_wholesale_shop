...
const MetricCard = ({ label, value, accent = false, compact = false }: { label: string; value: string; accent?: boolean; compact?: boolean }) => (
  <div className="rounded-2xl bg-slate-50 px-3 py-2">
    <p className={shell('uppercase tracking-[0.14em] text-slate-400', compact ? 'text-[9px]' : 'text-[10px]')}>{label}</p>
    <p className={shell('mt-1 wrap-break-word font-semibold leading-5', compact ? 'text-xs' : 'text-sm', accent ? 'text-emerald-700' : 'text-slate-900')}>{value}</p>
  </div>
);
                ...
                return <motion.div key={product.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.03 }} onClick={() => handleProductClick(product)} className="group flex min-h-110 cursor-pointer flex-col overflow-hidden rounded-[28px] border border-slate-200/70 bg-white shadow-sm transition-all hover:-translate-y-1 hover:shadow-md">
                  ...
                  <div className="flex flex-1 flex-col p-4 sm:p-5">
                    <div className="flex-1">
                      <h3 title={formatProductName(product.name)} className="line-clamp-4 wrap-break-word text-xs font-semibold leading-4 text-slate-900 sm:text-sm">{formatProductName(product.name)}</h3>
                      <div className="mt-3 min-w-0"><span className="inline-flex max-w-full wrap-break-word rounded-full bg-violet-100 px-3 py-1.5 text-[11px] font-medium text-violet-700">{product.category?.name || 'Без категории'}</span></div>
                      <div className="mt-4 grid grid-cols-3 gap-2"><MetricCard compact label="Продажа" value={shouldShowPrice(product) ? formatMoney(commerce.salePrice) : 'Скрыта'} /><MetricCard compact label="Закупка" value={formatMoney(commerce.purchasePrice)} /><MetricCard compact label="Рентаб." value={commerce.marginPercent > 0 ? `${commerce.marginPercent.toFixed(1)}%` : '0%'} accent /></div>
                    </div>
                    <div className="mt-5 flex items-start justify-between gap-3"><span className={shell('min-w-0 rounded-2xl px-3 py-2 text-left text-[11px] font-medium sm:text-xs', stockParts.isOutOfStock ? 'bg-rose-100 text-rose-700' : 'bg-emerald-100 text-emerald-700')}><span className="block wrap-break-word leading-4">Остаток: {stockParts.primary}</span>{stockParts.secondary ? <span className="mt-1 block wrap-break-word text-[10px] leading-4 opacity-90">{stockParts.secondary}</span> : null}</span><div className="min-w-0 text-right"><p className="text-[9px] uppercase tracking-[0.14em] text-slate-400">Прибыль</p><p className={shell('mt-1 wrap-break-word text-xs font-semibold leading-5', commerce.profitPerUnit >= 0 ? 'text-emerald-700' : 'text-rose-600')}>{formatMoney(commerce.profitPerUnit)}</p></div></div>
