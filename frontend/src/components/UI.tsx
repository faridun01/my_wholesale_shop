...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-900/50 p-4 backdrop-blur-sm">
      <div
        className={clsx(
          'relative w-full overflow-hidden rounded-[28px] bg-white shadow-2xl transition-all border border-white',
          maxWidthClasses[maxWidth]
        )}
      >
        <div className="flex items-start justify-between border-b border-slate-100 bg-[#f4f5fb] px-6 py-4">
          <div>
            {typeof title === 'string' ? (
          ...
          <button
            type="button"
            onClick={onClose}
            className="flex h-8 w-8 items-center justify-center rounded-full text-slate-400 hover:bg-slate-200/60 hover:text-slate-700 transition-colors"
          >
            <X size={18} />
          </button>
        </div>
